...
In short, a so-called _Mono-Repo_ is a (git) repository that houses multiple projects. Such projects are called workspaces or packages. In contrast, using multiple repositories that represent a particular project is a _Multi-Repo_ approach.  Of course, a combination of both approaches is possible. In my current job, we are multiple teams where each team has its own repositories. There are teams that pursue a _Multi-Repo_ approach and teams that believe in _Multi-Repo_. And there are teams that uses both because the technology that is part of the repository is also a factor to have in mind for decision making.
...
As an example, with the following command _react_ gets added to the package / workspace called &quot;awesome-package&quot; as dev dependency:
...
**TODO klären Dependency zu allen Packages hinzufügen: $ yarn add react —dev ?????**
...
You have to use [noHoist](https://yarnpkg.com/blog/2018/02/15/nohoist/) to use otherwise incompatible 3rd party dependencies working in the _Mono-Repo_ environment.
...
If you want to install _React_ as dependency to every package, execute the following command:
...
Consider the following npm script called &quot;test&quot;. The following commands show how to execute testing only on particular packages by using the _--scope_ flag along with globs. _lerna_ tries to execute _yarn test_ for every package that matches.
...
A fundamental question is whether to choose _npm_ or _yarn_. Since this topic is worth a separate article, I do not go into detail. If you are interested to find out more about the nitty-gritty details, take a look at my article on **TODO link zu Artikel**.

Anyway, as you can see with the &quot;cheat sheets&quot; above, you can switch easily between the different package managers just as you like.

**TODO --hoist lerna add sillyname@0.0.3 --hoist**


## Advanced Frontend Workflow Features and Commands

Even if you pick _yarn workspaces_ for dependency management, it totally makes sense to use _lerna_ as well. The reason is that _lerna_ provides high-level commands to optimize the management of multiple packages. For example, with one Lerna command you can iterate through all or particular packages, running a series of operations (such as linting, testing and building) on each package. Thereby, it compliments _yarn workspaces_ that takes over the dependency management process.
...
**TODO publishing / version: independet**
...
For a clean cache I executed one of the following commands depending on the used npm client:
...
The following command constitutes an example for approach 2 (_lerna_ with _npm_) and UC 1 (empty cache, no _node_modules_ folders, no lock files as precondition) for how I measured elapsed time:
```bash
...
With this approach, I try to find out if using yarn workspaces as part of a lerna configuration makes any difference regarding approach 4. Because there is no _lerna bootstrap_ required, the corresponding column is empty.
