...
A tab control is a crucial component of mobile apps. They enable users to navigate between screens or make different parts of content on a screen accessible by switching between views. 

As a mobile app developer, you typically utilise a segmented control on iOS.
...
On Android you normally use tab layouts according to Material Design Guidelines.
...
The goal of this article is to develop a React Native component that renders a segmented control on iOS devices and a tab component on Android devices. When it comes to the implementation, we reuse code for state management and data flow because they are platform-independent. Platform-specific code, e.g., due to different UI patterns, is used with the help of React Native's mechanisms like its Platform Module to distinguish between iOS and Android. 

The following animated gif demonstrate the tab control that is subject of this. You can find this project on Github or as public available Expo project.

![This demo shows how the component looks like on iOS and Android](../images/react-native-tab-control/demo.gif)

In this article, I want to focus on the important aspects of writing such a component. For this reason, I have broken the example down to the essentials. With respect to segmented control, I implemented a version using a motion animation as interaction concept to visualize the transition between active tabs. In the animated gif above you can watch how this UX pattern looks like in action (“iOS Variant: Motion Animation”). You can take a look at this Github project here or you can have a quick look via my Expo Snack.
...
Let us take a closer look at the different parts of the component. The interface of the `TabControl` component looks like this.
...
You have to pass in an array of `values` that are rendered as tab labels. The `onChange` prop is called when the user taps on a tab. The callback function gets the label of the active tab. In the demo project, I used the value to set the image source (`setImgSource`) in order to change the background image whenever the user taps on a tab.
...
Let's break this down even further. I utilized React Native's `Platform` API to render the component differently on both platforms. I created a tiny helper function (`isIos`) to perform platform checks throughout the component.
...
The `TabControl` component assumes that these properties exist on the stylesheet object. For reasons of clarity we skip the details of the iOS stylesheet object. 

`TabControl` is a stateful component that stores the selected index (`selectedIndex`). In addition, it defines function (`handleIndexChange`) to change the selected index and invoke the passed `onChange` callback (in our case to change the background images). 
...
The component renders a `SegmentedControl` component inside of a container with the aforementioned horizontal paddings. `values` and `renderSeparators` (a boolean flat to control whether separators should be rendered on iOS devices) along with the press handler and the index of the active tab are passed as props.

`SegmentedControl` is responsible to render a `Tab` component for every entry of the `tabValues` prop. The tab label and the tab press callback are passed on to the `Tab` component. `isActive`, `isFirst`, and `isLast` tell the `Tab` component the correct context to render the tabs differently (e.g., a bottom border for an active tab on Android). The context is determined by the current index and selected index. `renderLeftSeperators` represents a boolean flag whether a separator on the left side of the tab should be rendered (this is the implementation decision). I skip the implementation detail of the function `shouldRenderLeftSeparator`. Short, the separator for a tab is rendered for all tabs on the iOS platform if they are not the first or active tab or the previous tab is the active tab.
...
Finally, we have to take a look at the `useEffect` hook where the value of the `left` prop is calculated and then used by the animation (`Animated.timing()`). As you can see, useEffect's dependency array contains `containerWidth` and `activeTabIndex` (`numberValues` does not change). Whenever one of these values change, the animation is updated and the active tab indicator component moves on the horizontal axis within 250ms to the new position.
...
It renders for both OS a `Text` component with its associated styles (`tabTextStyle`). If the tab is active, additional styles are added (`activeTabTextStyle`). The container component, `OsSpecificTab`, is called with the `onPress` handler and a hint whether this tab is active. An array gets assigned to the `style` prop. If the tab represents the first or last tab additional styles are inserted to the array. Only for Android version styles are added in case the tab is active (`activeTabStyle`) to render the bottom border. As I described above, the implementation of the active tab for iOS is different and thus this information cannot be used on this layer of the component tree. 

`OsSpecificTab` is pretty simple, it just renders a `IosTab` or `AndroidTab` component and assigns all props to it.

Finally, let's take a look at the actual tab implementation. The implementation of `AnroidTab` utilizes [TouchableNativeFeedback](todo), which is an Android-only API to add a native look and feel. Instead of an odd-looking colored background, we assign a ripple effect to the `background` prop. The `child` container is an ordinary `View` that gets styled (`tabControlStyle`).
...
I have not succeeded to combine "iOS Variant 2 and 3" ( take a look at the animated gif above). As you can see with Apple Maps, you have a motion animation while switching the tabs and a scaling animation (i.e., the tab shrinks a bit) on tap. Feel free to test this for yourself with my [Github project](https://github.com/doppelmutzi/react-native-tab-control). With my approach based on absolute positioning, I had a layering (`z-index`) problem in a way that the tab was not positioned below the tab label. Here is the code for the iOS variant with a scaling animation. If you have an idea how to combine both variants, please let me know in the comments section.
...
During the preparation of this article I searched for existing React Native libraries that provide Android-like tabs and/or segmented controls. [This article](https://www.codeinwp.com/blog/react-native-component-libraries/) gives a good overview of component libraries in 2020. 
...
[React Native UI Kitten](https://akveo.github.io/react-native-ui-kitten/docs/components/tab-view/overview#tabview) offers a Tab View component that you can use for Android. It enables to use labels with and without icons. Again, there is no optimized version on iOS devices.
...
Until recently, React Native team offered a [segmented control component for iOS](https://reactnative.dev/docs/segmentedcontrolios). However, this component is deprecated. Instead, they recommend to use  [@react-native-community/segmented-control](https://github.com/react-native-community/segmented-control). At the time of this writing, the component only offers a iOS 12 design. [Nachos UI Kit](https://avocode.com/nachos-ui/docs/#!/Showcase/SegmentedControlButton) provides a rudimentary segmented control component.

The following Github projects implemented segmented control components. However, it appears that some of these projects are no longer maintained.
...
As you can see, the more complex part is the iOS version of this component. Since I'm mainly a iOS user, I might have missed a few design concepts for the Android version. Let me know in the comments section if there is more work to do for a better Android user experience. In comparison to a native implementation, there are of course a few shortcomings with this approach. Implementing and maintaining different designs for every operation system version is hard (e.g., the look and feel of the segmented control on iOS 12 and iOS 13 are different). I don't think that's necessary. However, in the end it depends on the project!

Do you think it would be useful if I would develop a library on Github based on the concepts described in this article? Let me know in the comments section what you think.

TODOs
- Kurzer Überblick über Libs
- Diagramm erstellen Komponenten
- swipe animation noch hinzufügen?
- mehr auf RN Libs eingehen: TouchableWithoutFeedback, Platform etc.
- native speaker blog article scannen bzgl. Floskeln vor Code Snippets
- aufzeigen/hinweisen auf die verschiedenen Möglichkeiten, plattform spezifische Dinge zu tun
    - ripple effect
        - https://callstack.github.io/react-native-paper/touchable-ripple.html
        - http://rnmk.xinthink.com/api/react-native-material-kit.ripple/
    - Animation bei iOS
            - ist haptisches feedback möglich?
